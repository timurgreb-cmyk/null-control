...
export default async function EmployeeTimesheetPage({
  ...
  const { data: profile } = await supabaseAdmin
    .from("profiles")
    .select("id, full_name, position, phone, pin_code, is_overtime_enabled")
    .eq("id", user.id)
    .single();
  ...
  return (
    <div className="p-4 pt-6 max-w-md mx-auto pb-28">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Мой табель</h1>
          <p className="text-slate-500 text-xs mt-0.5">Учет отработанных смен</p>
        </div>
        ...
        <div className="flex items-center bg-white border border-slate-200/80 rounded-2xl p-1 shadow-sm">
          ...
          <span className="text-xs font-bold text-slate-900 px-2 capitalize">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-primary/80 text-white rounded-3xl p-6 mb-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
            ...
            Смены в {format(selectedDate, "LLLL", { locale: ru })}е
          </span>
          <span className="bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[11px] font-bold px-2.5 py-0.5 rounded-full">
            {completedShifts} {completedShifts === 1 ? 'смена' : (completedShifts >= 2 && completedShifts <= 4 ? 'смены' : 'смен')}
          </span>
        </div>

        <div className="text-3xl font-black mb-5 tracking-tight">
          {completedShifts} <span className="text-lg font-bold text-slate-400">смен(ы)</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-3 text-center">
          <div>
            <p className="text-[10px] text-slate-300 font-semibold mb-0.5">ОТРАБОТАНО СМЕН</p>
            <p className="text-base font-black text-white">{completedShifts} смен</p>
          </div>
          <div className="border-l border-white/10 pl-2">
            <p className="text-[10px] text-slate-300 font-semibold mb-0.5">ОТРАБОТАНО ДНЕЙ</p>
            <p className="text-base font-black text-emerald-400">{dailyShifts.filter(s => s.status === 'complete').length} дн.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200/80 overflow-hidden mb-6">
        <div className="bg-slate-50/80 px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CalendarDays className="w-4 h-4 text-primary" />
            <h2 className="font-bold text-slate-900 text-sm">Смены за {monthTitle}</h2>
          </div>
            ...
            {dailyShifts.map((shift, idx) => (
              <div key={idx} className="p-4 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="font-bold text-slate-900 text-sm capitalize">{shift.formattedDay}</p>
                  ...
                  {shift.status === 'complete' && (
                    <div className="text-right">
                      <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/80 font-black text-xs px-2.5 py-1 rounded-xl">
                        {shift.shiftMultiplier ? `${shift.shiftMultiplier} смены` : '1 смена'}
                      </span>
                    </div>
                  )}
                  ...
                  {shift.status === 'missing_checkout' && (
                    <span className="bg-rose-50 text-rose-600 border border-rose-100 font-bold text-xs px-2.5 py-1 rounded-xl flex items-center gap-1">
                ...
                <div className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-xs">
                  <div className="flex items-center gap-1.5 text-slate-600">
                    <span className="text-slate-400 font-medium">Приход:</span>
                    <span className="font-bold text-slate-900">{shift.formattedFirstIn}</span>
                  </div>
                  <div className="w-4 h-[1px] bg-slate-300" />
                  <div className="flex items-center gap-1.5 text-slate-600">
                    <span className="text-slate-400 font-medium">Уход:</span>
                    <span className="font-bold text-slate-900">{shift.formattedLastOut}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200/80 p-5 space-y-4">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div>
            <h3 className="font-black text-slate-900 text-base">{profile?.full_name || "Сотрудник"}</h3>
            <p className="text-xs text-slate-500">{profile?.position || "Сотрудник пекарни"}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
            <p className="text-slate-400 font-semibold mb-0.5">Телефон</p>
            <p className="font-bold text-slate-900">{profile?.phone || "Не указан"}</p>
          </div>
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
            <p className="text-slate-400 font-semibold mb-0.5">PIN для входа</p>
            <p className="font-mono font-bold text-slate-900 tracking-wider">{profile?.pin_code || "—"}</p>
          </div>
        </div>
        ...
        <form action={logout}>
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2 p-3.5 bg-rose-50 text-rose-600 hover:bg-rose-100 border border-rose-100 rounded-2xl font-bold text-sm active:scale-95 transition-all mt-2"
          >
